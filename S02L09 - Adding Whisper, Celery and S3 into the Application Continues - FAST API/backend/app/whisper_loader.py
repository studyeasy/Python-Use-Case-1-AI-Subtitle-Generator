...
# Where huggingface_hub stores model files. Set via HF_HOME, otherwise the
# default cache path used inside the container image.
...
def _default_log(msg: str) -> None:
    # Use the module logger if it has handlers (runtime); otherwise print so
    # the build-time invocation still shows up in `docker build` output.
...
class CacheWatcher:
    ...
    def _run(self) -> None:
        # downloaded_mb     = total since the watcher started (cumulative)
        # last_logged_mb    = downloaded_mb at the previous log line
        # since_last_mb     = growth between this poll and last log
        last_logged_mb = 0.0
        last_log_t = self.t0
        first_emit = True
        ...
        while not self._stop.is_set():
            ...
            if first_emit and downloaded_mb > 0:
                ...
                # Heartbeat fired but no significant new bytes. Distinguish
                # "stalled (no growth at all)" from "trickle (some growth, just
                # below the chunk threshold)" — both useful, different signals.
...
def download_and_load(
    ...
    if local is not None:
        # Happy path: model files were baked into the image with curl. No HF
        # interaction, no Xet, no network — instant cold start.
