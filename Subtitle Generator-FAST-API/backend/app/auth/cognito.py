"""AWS Cognito integration for the FastAPI backend.

The frontend keeps talking only to this backend. This module brokers Cognito
...
def _map_cognito_error(exc: ClientError, fallback: str) -> HTTPException:
    ...
    if code in {"UsernameExistsException", "AliasExistsException"}:
        return HTTPException(
            ...
            detail="User exists with same email",
        )
    if code == "UserNotConfirmedException":
        return HTTPException(
            status_code=status.HTTP_403_FORBIDDEN,
            detail="User is not confirmed. Confirm the account before logging in.",
        )
    if code in {
...
def decode_token(token: str) -> dict[str, Any]:
    ...
    if client_id != settings.cognito_app_client_id:
        raise HTTPException(
            status_code=status.HTTP_401_UNAUTHORIZED,
            detail="Invalid Cognito app client",
            headers={"WWW-Authenticate": "Bearer"},
        )
...
def _merge_user_attributes(claims: dict[str, Any], access_token: str) -> dict[str, Any]:
    """Add profile attributes from Cognito GetUser when the token permits it."""
    try:
        response = get_cognito_client().get_user(AccessToken=access_token)
...
def get_current_user(
    ...
    return _merge_user_attributes(claims, credentials.credentials)
...
class CognitoClient:
    ...
    async def logout(refresh_token: str) -> None:
        try:
            ...
            get_cognito_client().revoke_token(**kwargs)
        except ClientError as exc:
    ...
    async def register(
        *,
        ...
        try:
            response = get_cognito_client().sign_up(**kwargs)
            confirmed = bool(response.get("UserConfirmed"))
            if settings.cognito_auto_confirm_sign_up and not confirmed:
                get_cognito_client().admin_confirm_sign_up(
                    UserPoolId=settings.cognito_user_pool_id,
                    Username=email,
                )
                get_cognito_client().admin_update_user_attributes(
                    UserPoolId=settings.cognito_user_pool_id,
                    Username=email,
    ...
    @staticmethod
    async def confirm_sign_up(email: str, code: str) -> None:
        kwargs: dict[str, Any] = {
            "ClientId": settings.cognito_app_client_id,
            "Username": email,
        ...
        try:
            get_cognito_client().confirm_sign_up(**kwargs)
        except ClientError as exc:
            raise _map_cognito_error(exc, "Could not confirm user") from exc
        except BotoCoreError as exc:
            raise HTTPException(
                status_code=status.HTTP_502_BAD_GATEWAY,
                detail=f"Could not confirm user: {exc}",
            ) from exc

    @staticmethod
    async def resend_confirmation_code(email: str) -> None:
        kwargs: dict[str, Any] = {
            "ClientId": settings.cognito_app_client_id,
            "Username": email,
        }
        ...
        try:
            get_cognito_client().resend_confirmation_code(**kwargs)
        except ClientError as exc:
            raise _map_cognito_error(exc, "Could not resend confirmation code") from exc
        except BotoCoreError as exc:
            raise HTTPException(
                status_code=status.HTTP_502_BAD_GATEWAY,
                detail=f"Could not resend confirmation code: {exc}",
            ) from exc
    ...
    async def forgot_password(email: str) -> None:
        ...
        try:
            get_cognito_client().forgot_password(**kwargs)
        except ClientError as exc:
            raise _map_cognito_error(exc, "Could not start password reset") from exc
        except BotoCoreError as exc:
            raise HTTPException(
                status_code=status.HTTP_502_BAD_GATEWAY,
                detail=f"Could not start password reset: {exc}",
            ) from exc
    ...
    async def confirm_forgot_password(
        ...
        try:
            get_cognito_client().confirm_forgot_password(**kwargs)
        except ClientError as exc:
